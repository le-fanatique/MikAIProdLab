...
export const ACTION_REGISTRY = {
  ...
  createShotAtPosition: {
    ...
    writeSemantics: "insertPerItem",
    notes: [
      "SHOTPROMPT.DERIVE.1 — `shotPrompt` no longer appears here: `ProposedShot` carries no such field, and the action no longer derives one from `description`/`actionPitch` via `resolveShotPromptWithDefault`. A directed Shot inserted this way starts with no `shot_prompt` at all — this is the UC1 case that used to get one fabricated on every insertion.",
