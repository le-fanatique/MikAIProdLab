...
function describeConnectionFailure(err: unknown, baseUrl: string): string {
  const cause = extractFetchErrorCause(err);
  return cause
    ? `Cannot connect to LLM server at ${baseUrl}: ${cause}. Check your settings.`
    : `Cannot connect to LLM server at ${baseUrl}. Check your settings.`;
}

/**
 * Builds the failure message for `fetchOpenAICompatibleModelNames`'s fetch
 * catch. Does not fit `describeConnectionFailure`'s pattern as-is: that one's
 * no-cause text ("Cannot connect to LLM server at ... Check your settings.")
 * differs from today's "Could not reach LLM server." — reusing it would
 * change the no-cause wording, which `LLM.ERROR.CAUSE.2` forbids.
 */
export function describeModelListingFailure(err: unknown): string {
  const cause = extractFetchErrorCause(err);
  return cause
    ? `Could not reach LLM server: ${cause}.`
...
export async function callOpenAICompatibleChat(
  ...
  try {
    ...
    throw new Error(describeConnectionFailure(err, config.baseUrl));
  } finally {
    clearTimeout(timer);
  }
...
export async function fetchOpenAICompatibleModelNames(
  ...
  try {
    ...
    throw new Error(describeModelListingFailure(err));
  } finally {
    clearTimeout(timer);
  }
