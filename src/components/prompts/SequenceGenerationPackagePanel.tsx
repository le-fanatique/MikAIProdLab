...
export default async function SequenceGenerationPackagePanel({
  ...
  // Shot count and known duration only — no warnings list here (see
  // "Diagnostics" sub-panel below for those).
  const summaryLabel = `Sequence Generation Package — ${pkg.shotCount} shot${pkg.shotCount !== 1 ? "s" : ""} · ${pkg.totalKnownDurationSeconds.toFixed(1)}s known (${pkg.knownDurationShotCount}/${pkg.shotCount} timed)`;

  return (
    <Card>
      <Collapsible label={summaryLabel}>
        <div className="flex flex-col gap-3">
          ...
          <form method="get" className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-[#a4abb2]">
