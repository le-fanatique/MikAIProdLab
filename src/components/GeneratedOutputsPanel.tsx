...
export default function GeneratedOutputsPanel({ outputs }: Props) {
  if (outputs.length === 0) {
    return (
          ...
          >
            ...
            <div className="w-full aspect-video bg-[#141618] rounded overflow-hidden flex items-center justify-center">
              {isImage ? (
                ...
                <p className="text-xs text-[#4b5158] font-mono">.{ext || "bin"}</p>
              )}
            </div>
            ...
            <div className="flex flex-col gap-1 px-1">
              <div className="flex items-center gap-1.5 flex-wrap">
                {item.workflowKind && (
                  <WorkflowKindBadge
                    kind={
                      item.workflowKind === "image" || item.workflowKind === "video"
                        ? item.workflowKind
                        : "image"
                    }
                  />
