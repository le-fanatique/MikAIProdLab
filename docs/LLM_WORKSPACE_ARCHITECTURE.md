...
| B7f-g | The two remaining migrations | **Neither is a migration yet**: `createSelectedAssets` and `createGeneratedSequences` have no descriptor, and `assetExtraction` additionally waits on B7c-n1 (boolean / multi-choice inputs) and B7c-n2 (project-scope collection variables). Each needs its own descriptor ticket first. | none |
