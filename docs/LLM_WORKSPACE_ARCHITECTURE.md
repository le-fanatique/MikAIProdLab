...
| **S1** | **Schema — Asset Bible freshness, and asset sourcing metadata** | **Authorized by the user 2026-08-17** — see `docs/ARCHITECTURE_DECISIONS.md`, "Four Arbitrations Taken 2026-08-17". Two separate tickets. The sourcing one **must ship its display surface with its columns**, or it recreates the same silent loss one layer down. | none |
...
| **S6** | **UC1 on the Sequence page** | Not scheduled. `shot.insertDirected` is bench-only, on the precedent the user set for UC3 on 2026-08-17: he judges the answers' quality before a production surface is built for them. | **surfaces UC1** |
