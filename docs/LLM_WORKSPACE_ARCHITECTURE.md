...
**What the first consumer exercised, and what it did not.** `lighting.fromImage`
(B16b, `ef470fb`) proved the declaration end to end — resolution, byte
re-validation, the `{images}` block, the multimodal call, and a commit that
writes one column. It did **not** exercise an ordered selection: the bench's
checkbox control serializes in DOM order, so its `R1..Rn` follow the list's
display order, never the order the user ticked boxes in. That is a limit of
**that control**, not of the format — the format keys by selection ordinal and
will honour any order a surface can express. B20's Reference Board, which lets
the user order images by hand, has its own surface and is not constrained by it.

**A brick B16c found missing, named rather than absorbed.** `PreconditionRef`
(`types.ts`) has three variants — `anchorField`, `parameter`, `variable` — and
**none can gate on a non-array variable's own emptiness**. `sequence.lightingDirected`
needs exactly that and therefore ships with no precondition at all:

- `{ anchorField: "lighting" }` reads the key `mergeAnchorFields` flattens out of
  the resolved variable. `SeqLightingData` only carries a `lighting` key when
  `source === "own"`, so this ref would refuse a Sequence whose lighting is
  **inherited from its environment Assets** — the very case §5.9 built the
  feature for;
- `{ variable: "SEQ.LIGHTING" }` reaches `isVariableNonEmpty`, which **throws**
  unless the value is an array. It never is.

Consequence in production: a Sequence with lighting nowhere reaches the model
instead of being refused — one wasted call, no fabricated value (the render form
states plainly that nothing is recorded). The same gap is why an **empty
director's note** cannot be formally refused either: `intent.freeText` is not an
`anchorField`, a `parameter` or a `variable`, so no ref can name it.

Per §11.3's governing rule this is a ticket to write, not a reason to shrug —
but it is not urgent (one operation wants it), and widening a frozen union is
format work, so it belongs to a supervisor-owned ticket rather than to whichever
executor ticket next trips over it.
...
**The output discipline (B13b, `f1ce136`) reports and never gates.** `inspect`
measures the four things §5.6 said nothing measured — the 60–100 word budget
with its 150 hard cap, the one-primary-camera rule, the 9-image and 12-file tag
caps — plus a missing lighting, which the guide calls the highest-leverage
single element. Every result is a `ConformationFinding`, and the type has **no
`"error"` severity**: §5.4 forbids turning formatting into a rule the user must
obey, so the module contains no `throw`, no `{ok:false}` and no truncation
anywhere. A prompt over budget is still a prompt, handed back whole with a note.

Two shapes there are deliberate. `cameraPhrases` is only ever length-checked,
never parsed — decision 1 above, protected. And the file cap takes a
**number**, not a per-family list: `shot_reference_videos` has no role column
and audio has no entity at all (§5.6), so declaring a shape for either today
would be inventing one; when B17 brings them, the caller passes a larger number
...
Their specifications were **moved to `docs/archive/LLM_WORKSPACE_PLANNING.md`
on 2026-08-21**. What they decided is in sections 3 and 4 above, which are the
durable reference; what they planned is in the archive.
...
### C1/C2 re-measured — archived

That section declared its own figures stale, and Chantier 2 completed on
2026-08-20. **Moved to `docs/archive/LLM_WORKSPACE_PLANNING.md` on
2026-08-21.** The outcome is recorded in `docs/PROJECT_STATE.md`.
