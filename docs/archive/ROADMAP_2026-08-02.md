# Roadmap consolidée du 2 août 2026 — archive

**Archivée le 2026-08-21. Dans aucun contrat de lecture.**

Ceci est la roadmap telle qu'elle existait avant le chantier LLM Workspace,
avant le nettoyage du Chantier 2 et avant la refonte caméra. Elle a été
consolidée le 2 août 2026, puis rapiécée le 20 août par un bloc de tête qui
demandait de lire tout le reste « comme archive des intentions » — sans que la
structure le signale, et en gardant des titres impératifs (`Maintenant`,
`Priorité absolue`, `à valider avant le prochain ticket`) sur des sections
closes depuis des semaines.

Elle est déplacée ici entière, sans rien supprimer. Elle garde deux usages
légitimes :

- **le registre de livraison** — quel ticket a livré quoi, à quel commit. Les
  registres `STYLE.1`, Seedance MVP, Camera Lab et SEQGEN sont ici, et nulle
  part ailleurs sous cette forme ;
- **la mémoire des arbitrages de backlog** — ce qui a été retiré, fusionné ou
  remplacé, et pourquoi.

Elle n'a plus aucune autorité sur les priorités. Pour ce qui est ouvert
aujourd'hui, lire `docs/ROADMAP.md` ; pour l'état du produit,
`docs/PROJECT_STATE.md`.

Le document précisait lui-même, au point `0bis`, qu'il n'avait pas été
réconcilié : « la roadmap date de la période 2026-08-02 et une réconciliation
complète dépasse ce que cette mise à jour a vérifié ». C'est cette
réconciliation qui a été faite le 2026-08-21, et son résultat vit dans le
nouveau `docs/ROADMAP.md`.
